...
|:------|:--------:|:------|:------|
| v10.5.0|**02/02/26**| Solax & Octopus Settings.yaml (dashboard) <br>  automations_5000-5005.yaml <br> packages\solax_export\template_discharge_magic.yaml<br> packages\solax_loads\template_solax_extras.yaml <br> packages\solax_realtime\templates.yaml <br> NEW FILE: <br> packages\octopus_dispatches\input_select.yaml <br> packages\zappi_loads\template_zappi_connected.yaml| Dashboard improvements <br> 5000 fixed a minor bug with calculating limited charge target if EV is reconnected before 12pm. 5001 addressed a race condition in variables block for smoother transitions when companion charging <br> Moved solax_local_battery_power_adjusted to extras package |
| v10.4.0|**22/01/26**| packages\octopus_dispatches\template_hh_offpeak.yaml |Improved contiguous slots when exiting the last slot of the HH period when STATIC_OFF_PEAK_ENABLED = false|
| v10.3.0|**21/01/26**| Solax & Octopus Settings.yaml (dashboard) <br>  automations_5000-5005.yaml <br> packages\octopus_dispatches\input_boolean.yaml <br> packages\octopus_dispatches\input_number.yaml <br> packages\octopus_dispatches\template_hh_offpeak.yaml <br> packages\octopus_dispatches\template_octopus_dispatch.yaml <br> packages\octopus_dispatches\template_octopus_tariff.yaml <br> packages\solax_export\template_discharge_magic.yaml <br> packages\solax_loads\template_solax_extras.yaml <br> packages\solax_zappi_octopus_notifications\input_select.yaml| Companion charging in off peak slots <br> battery heating tracking improvements|
| v10.2.0|**11/01/26**| Solax & Octopus Settings.yaml (dashboard) <br>  automations_5000-5005.yaml <br> packages\connected_ev_tracking\template_ev_mapper.yaml <br> packages\octopus_dispatches\template_octopus_dispatch.yaml <br> packages\octopus_dispatches\template_hh_offpeak.yaml <br> packages\solax_control\rest_command.yaml <br> packages\solax_loads\powercalc.yaml <br> packages\solax_loads\template_solax_extras.yaml <br> packages\solax_loads\utilitymeter.yaml <br> packages\solax_realtime\templates.yaml| Track battery heating, new binary_sensor for tracking offpeak HH slots during peak times (future automations coming to utilise this sensor) + various improvements/bug fixes to keep logs clean|
| v10.1.6|**31/12/25**| Solax & Octopus Settings.yaml (dashboard) <br>  automations_5000-5005.yam | Changes to dashboard and small bug fix to make sure max 6hr allowance is used if the EV was connected prior to 12 noon|
